...
| 11 | Bedrock boundary | `client.py` is the **sole owner of Bedrock runtime client construction** (the shared `bedrock` object). Every application/domain module — `retrieval.py`, `planner.py`, `subjects.py`, `reranker.py`, `eval.py`, `judges.py` — imports `bedrock` (and `embed_text()`) from `client.py` and constructs no Bedrock client of its own. This is an architectural rule (testability, one place to swap providers), not an incidental detail of the current code. It does **not** claim `client.py` owns every AWS client: `manage.py` intentionally owns a separate OpenSearch Serverless **control-plane** client (decision 11 applies to Bedrock only — see the note below and §11 item 4). |

Decision 11's boundary, concretely:

```
Bedrock runtime / RAG infrastructure
    -> client.py
...
OpenSearch Serverless control plane / collection lifecycle
    -> manage.py   (its own client; a separate capability, not Bedrock)
```
...
`client.py` and `judges.py` are intentional project dependencies and stay unchanged in behavior and public API; the same holds for `subjects.py` / `subjects.json` and the `data/` corpus. ("Unchanged" is about behavior and API — `judges.py`'s Bedrock *client wiring* was brought into line with decision 11, exactly as `subjects.py`'s was.)

**On decision 11:** `subjects.py` and `judges.py` each originally built their own `boto3.client("bedrock-runtime", ...)`; both were refactored to `from client import bedrock` (confirmed in each file's history: `-import boto3` / `-bedrock = boto3.client(...)` → `+from client import bedrock`), with every other line, including `MODEL_ID`, left untouched. `planner.py` and `reranker.py` were written directly against `client.bedrock` and never had a version of their own in this repo. Repo-wide, `boto3.client("bedrock-runtime", ...)` now occurs exactly once, in `client.py`. `manage.py` is a **separate, still-standing exception for a different capability**: its own `boto3.Session`/`.client("opensearchserverless")` construct the OpenSearch Serverless control-plane client (`batch_get_collection`, `delete_collection` — collection lifecycle), which `client.py` deliberately does not expose (it exposes only the data-plane `opensearch_client()`). That exception does not touch Bedrock and is not something decision 11 asks to close.
...
**`subjects.py` / `subjects.json` — taxonomy and tagging behavior unchanged; client wiring refactored.** Single source of truth for the vocabulary. Uses the shared `client.bedrock` (no client of its own — see decision 11). The tagger reads only the first 6000 characters of each article (31 of 32 are longer), and every chunk inherits its article's tags; this is inherent to article-level tagging, and the eval's `completeness` would expose a filtered-out answer.
...
3. **`MODEL_ID` is not centralized.** `judges.py`, `planner.py`, `reranker.py`, and `retrieval.py` each read `os.environ["BEDROCK_MODEL_ID"]` directly instead of the already-validated `config.BEDROCK_MODEL_ID`. Out of scope for decision 11 (which covers Bedrock *client construction*, not model-ID configuration); left as a separate, deliberate future decision so it can be applied to all four modules uniformly rather than piecemeal.
4. **`manage.py` builds its own AWS session/client** (`_aoss_session()`, mirroring `client.py`'s private helper) — for a different capability than decision 11 covers: OpenSearch Serverless **control-plane** operations (`batch_get_collection`, `delete_collection`) that `client.py` deliberately does not expose (it only exposes the **data-plane** client via `opensearch_client()`). This is a reviewed, accepted, standing design choice, not an oversight, and it does not involve Bedrock at all.
